...
library(ggpubr)
	...
	loadingsplot <- function(pca.obj)  
	{
	    ...
	    df = as.data.frame(pca.obj$loadings[,1:m]) # get first m columns which are the m PCs
	    ...
	    df = as.data.frame(pca.obj$rotation[,1:m]) # get first m columns which are the m PCs
	    setDT(df, keep.rownames=TRUE)[]
	    names(df)[names(df) == "rn"] <- "Variable"
	...
	ggbiplot <- function (pcobj, choices = 1:2, scale = 1, plot.obs = TRUE, xlim=FALSE, ylim=FALSE,
	                      ...
	                      ...) 
	{
	  # code modified from https://github.com/vqv/ggbiplot/tree/experimental
	  ...
	  if(length(xlim)>1){
	    g <- g + expand_limits(x=xlim)
	  }
	  if(length(ylim)>1){
	    g <- g + expand_limits(y=ylim)
	  }
	  return(g)
	}
