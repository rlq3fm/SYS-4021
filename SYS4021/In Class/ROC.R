...
plot.roc.gg  <- function(p, r, modelname, main = "ROC Curve")
{
  tp <- truepos(p, r)
  tn <- trueneg(p, r)
  plot <- ggplot() +
    geom_line(aes(x=1-tn, y=tp, color=modelname), lwd=1.5) + 
    geom_line(aes(x=c(0,1),y=c(0,1)), color="black", lwd=1.5) + 
    xlab("False Positive Rate") + 
    ylab("True Positive Rate") + ggtitle(main) +
    theme(text = element_text(size=16))
}

lines.roc.gg  <- function(plot, p, r, modelname)
{
  tp <- truepos(p, r)
  tn <- trueneg(p, r)
  plot + geom_line(aes(x=1-tn, y=tp, color = modelname), lwd=1.5)
}
