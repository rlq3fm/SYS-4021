...
pc.glm <- function(pc.obj, goal.percent, response)
{
  ...
  if ("scores" %in% names(pc.obj)){ # princomp object
    ...
    glm(r ~ ., data = data.frame(pc.obj$x[,1:k], r = response), family = binomial)
  }
}
...
pc.null <- function(pc.obj, goal.percent, response)
{
	...
	if ("scores" %in% names(pc.obj)){ # princomp object
	  ...
	  glm(r ~ 1, data = data.frame(pc.obj$x[,1:k], r = response), family = binomial)
	}
}
...
predict.pc.glm <- function(pcglm.obj, pc.obj, ndata, type = "response")
{
	...
	if("loadings" %in% names(pc.obj)){ #princomp object
	  PC <- data.frame(M %*% pc.obj$loadings)
	} else { # prcomp object
	  PC <- data.frame(M %*% pc.obj$rotation)
	}
